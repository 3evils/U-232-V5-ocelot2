...
while ($hit_and_run_arr = mysqli_fetch_assoc($hit_and_run_rez)) {
    //=== Ocelot Tracker or Default Announce
        $Ocelot_Seed = (OCELOT_TRACKER === true ? $hit_and_run_arr['active'] !== 1 : $hit_and_run_arr['seeder'] !== 'yes');
        $Uid_ID = (OCELOT_TRACKER === true ? $hit_and_run_arr['uid'] : $hit_and_run_arr['userid']);
        $S_date =(OCELOT_TRACKER === true ? $hit_and_run_arr['started'] : $hit_and_run_arr['start_date']);
        $T_ID = (OCELOT_TRACKER === true ? $hit_and_run_arr['fid'] : $hit_and_run_arr['torrentid']);
        $C_Date = (OCELOT_TRACKER === true ? $hit_and_run_arr['completedtime'] : $hit_and_run_arr['complete_date']);
