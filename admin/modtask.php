    ...
    if ((isset($_POST['reset_torrent_pass'])) && ($_POST['reset_torrent_pass'])) {
        ...
        if (OCELOT_TRACKER) {
            if ($user['torrent_pass'] != '') {
                ...
                $user['torrent_pass'] = $newpasskey;
                Tracker::update_tracker('add_user', array('passkey' => rawurlencode($newpasskey), 'id' => $user['id'], 'visible' => ($user['privacy'] != 'normal' ? 1 : 0), 'free_switch' => $user['free_switch']));
            }
        }
    }
    //== seedbonus
