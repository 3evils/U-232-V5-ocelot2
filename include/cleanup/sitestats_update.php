...
function docleanup($data)
{
    ...
    ignore_user_abort(1);
    //== 09 Stats
    $OCELOT_Seeder = mysqli_fetch_assoc(sql_query("SELECT sum(seeders) AS seeders FROM torrents")) or sqlerr(__FILE__, __LINE__);
    $OCELOT_Leecher = mysqli_fetch_assoc(sql_query("SELECT sum(leechers) AS leechers FROM torrents")) or sqlerr(__FILE__, __LINE__);
    ...
    $seeders = (int)$OCELOT_Seeder['seeders'];
    $leechers = (int)$OCELOT_Leecher['leechers'];
