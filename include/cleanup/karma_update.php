...
function docleanup($data)
{
    ...
    if ($INSTALLER09['seedbonus_on'] == 1) {
     $What_id = (OCELOT_TRACKER == true ? 'fid' : 'torrent');
     $What_user_id = (OCELOT_TRACKER == true ? 'uid' : 'userid');
     $What_Table = (OCELOT_TRACKER == true ? 'xbt_files_users' : 'peers');
     $What_Where = (OCELOT_TRACKER == true ? "`left` = 0 AND `active` = 1" : "seeder = 'yes' AND connectable = 'yes'");
    ...
    if (mysqli_num_rows($res) > 0) {
        while ($arr = mysqli_fetch_assoc($res)) {
            ...
            $Buffer_User = (OCELOT_TRACKER == true ? $arr['uid'] : $arr['userid']);
