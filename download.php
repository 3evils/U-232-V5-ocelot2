...
if (isset($_GET['slot'])) {
    ...
    elseif ($_GET['slot'] == 'double') {
        ...
        $CURUSER['freeslots'] = ($CURUSER['freeslots'] - 1);
        sql_query('UPDATE users SET freeslots = freeslots - 1 WHERE id = ' . sqlesc($CURUSER['id']) . ' LIMIT 1') or sqlerr(__FILE__, __LINE__);
        if ($used_slot && $slot['free'] == 'yes') sql_query('UPDATE freeslots SET doubleup = "yes", addedup = ' . $added . ' WHERE torrentid = ' . sqlesc($id) . ' AND userid = ' . sqlesc($CURUSER['id']) . ' AND free = "yes"') or sqlerr(__FILE__, __LINE__);
